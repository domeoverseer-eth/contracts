...
contract DomeTreasury is Ownable {
    ...
    address public sDOME;
    uint public sDOMEQueue; // Delays change to sDOME address
    ...
    /**
        @notice allow approved address to deposit an asset for DOME
        @param _amount uint
    ...
    /**
        @notice allow approved address to burn DOME for reserves
        @param _amount uint
        @param _token address
     */
    ...
    /**
        @notice allow approved address to repay borrowed reserves with DOME
        @param _amount uint
     */
    function repayDebtWithDOME( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IDOMEERC20( DOME ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, DOME, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( DOME ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( DOME ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns DOME valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SDOME ) { // 9
            sDOMEQueue = block.number.add( blocksNeededForQueue );
        } else return false;
